...
const announcements: FastifyPluginAsync = async (fastify, opts): Promise<void> => {
  ...
  fastify.put<{ Params: { id: string }; Body: any }>('/:id', async function (request, reply) {
    const announcement = await AnnouncementService.update(request.params.id, request.body)
    return announcement
  })

  fastify.delete<{ Params: { id: string } }>('/:id', async function (request, reply) {
